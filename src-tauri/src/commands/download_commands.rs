 ...
 * Important Details: Downloads are streamed to disk in 8KB chunks. Progress events
 *   are emitted every 1% or every 500ms (whichever comes first) to avoid flooding
 *   the IPC channel. The download directory is $APP_DATA/models/gguf/.
 */
...
const DEFAULT_MODEL_SIZE_ESTIMATE: u64 = 2_000_000_000; /* ~2 GB */
...
pub fn download_model(
    ...
    /* Skip if already downloaded and looks complete (>100MB) */
    ...
    tracing::info!("Downloading model: {model_name} from {download_url}");
    ...
    std::thread::spawn(move || {
        if let Err(e) = do_download(&app_clone, &url_owned, &name_owned, &path_owned) {
...
fn do_download(
    ...
    let total = response.content_length().unwrap_or(DEFAULT_MODEL_SIZE_ESTIMATE);

    /* Write to a temp file first, rename on completion (atomic-ish) */
    ...
    /* Stream in 64KB chunks for good throughput */
    ...
    loop {
        ...
        /* Emit progress every 1% or every 500ms to avoid flooding IPC */
