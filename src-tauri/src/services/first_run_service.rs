 ...
 * Important Details: Only runs if zero notebooks exist in the database. The sample
 *   notebook contains 2 notes demonstrating wiki-links and formatting. Idempotent:
...
use crate::database::repository::{notebook_repository, note_repository};
use crate::error::AppResult;


/// Create a sample notebook if the database is empty (first run).
pub fn ensure_sample_notebook(conn: &Connection) -> AppResult<()> {
    let notebooks = notebook_repository::list_all(conn)?;
    ...
    note_repository::create(conn, CreateNote {
        notebook_id: notebook.id.clone(),
        title: Some("Welcome to NotebookLab".to_string()),
        content: Some(r#"# Welcome to NotebookLab
...
See the [[Research Notes]] for an example of how notes connect."#.to_string()),
    })?;

    note_repository::create(conn, CreateNote {
        notebook_id: notebook.id.clone(),
        title: Some("Research Notes".to_string()),
        content: Some(r#"# Research Notes
