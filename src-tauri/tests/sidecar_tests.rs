...
fn generate_session_key_has_correct_prefix() {
    ...
    assert!(key.starts_with("nbl-"), "Key should start with 'nbl-' prefix");
}
...
fn generate_session_key_has_sufficient_length() {
    ...
    assert!(key.len() >= 36, "Key should be at least 36 chars, got {}", key.len());
}
...
fn find_model_files_finds_gguf_files() {
    ...
    let sizes: Vec<u64> = models.iter()
        .map(|p| std::fs::metadata(p).unwrap().len())
        .collect();
    assert!(sizes[0] <= sizes[1], "Should be sorted smallest first");

    /* Should not include the .txt file */
    assert!(models.iter().all(|p: &std::path::PathBuf| p.extension().unwrap() == "gguf"));
...
fn sidecar_manager_try_transition() {
    ...
    assert!(mgr.try_transition(sidecar_service::STATE_STOPPED, sidecar_service::STATE_STARTING));

    /* Should fail: trying STOPPED -> STARTING again while in STARTING */
    assert!(!mgr.try_transition(sidecar_service::STATE_STOPPED, sidecar_service::STATE_STARTING));

    /* Should succeed: STARTING -> READY */
    assert!(mgr.try_transition(sidecar_service::STATE_STARTING, sidecar_service::STATE_READY));
    assert!(mgr.is_running());
}
